  ...
  "monthDecryptor" should {
    "decrypt to Month values" in {
      ...
      (secureGCMCipher.decrypt[Month](_: String, _: String)(_: TextAndKey, _: Converter[Month]))
        .expects(encryptedValue.value, encryptedValue.nonce, textAndKey, monthLoader).returning(monthValue)

      monthDecryptor.decrypt(encryptedValue) shouldBe monthValue
    }
  }
}
