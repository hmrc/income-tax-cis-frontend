...
class SecureGCMCipherSpec extends UnitTest {
  ...
  "encrypt" should {
    "return plain text when turned off" in {
      val encryptorWithNoCrypt = new SecureGCMCipher()(new MockAppConfig().config(encrypt = false))
      val encryptedText = encryptorWithNoCrypt.encrypt(textToEncrypt)
    ...
    "return an EncryptionDecryptionError if the secret key is an invalid type" in {
      ...
      val secureGCMEncryptor = new SecureGCMCipher {
        override val ALGORITHM_KEY: String = "DES"
      }
      val encryptedAttempt = intercept[EncryptionDecryptionException](
        secureGCMEncryptor.generateCipherText(textToEncrypt, associatedText.getBytes,
    ...
    "return an EncryptionDecryptionError if the alg is invalid" in {
      ...
      val encryptedAttempt = intercept[EncryptionDecryptionException](
        secureGCMEncryptor.encrypt((textToEncrypt, associatedText, secretKey))
      )
    ...
    "return an EncryptionDecryptionError if a IllegalBlockSizeException is thrown" in {
      ...
      assert(encryptedAttempt.failureReason.contains("Error occurred due to block size"))
    }
  ...
  "decrypt" should {
    "return plain text when turned off" in {
      val encryptorWithNoCrypt = new SecureGCMCipher()(new MockAppConfig().config(encrypt = false))
      val encryptedText = encryptorWithNoCrypt.decrypt(textToEncrypt, "nonce")
    ...
    "return a EncryptionDecryptionException if the encryptedValue is different" in {
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        underTest.decrypt(Base64.getEncoder.encodeToString("different values".getBytes), encryptedTextTest.nonce)
      )
      assert(decryptedAttempt.failureReason.contains("Error occurred due to padding scheme"))
    }
    ...
    "return an EncryptionDecryptionError if the secret key is an invalid type" in {
      ...
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        secureGCMEncryptor.decryptCipherText(encryptedTextTest.value, associatedText.getBytes,
          new GCMParameterSpec(96, "hjdfbhvbhvbvjvjfvb".getBytes), key)
      )
      assert(decryptedAttempt.failureReason.contains("Key being used is not valid." +
        " It could be due to invalid encoding, wrong length or uninitialized"))
    ...
    "return an EncryptionDecryptionError if the alg is invalid" in {
      ...
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        secureGCMEncryptor.decrypt(encryptedTextTest.value, encryptedTextTest.nonce)
      )
    ...
    "return an EncryptionDecryptionError if a IllegalBlockSizeException is thrown" in {
      ...
      assert(decryptedAttempt.failureReason.contains("Error occurred due to block size"))
    }
