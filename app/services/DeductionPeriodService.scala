...
import models.mongo.{CYAPeriodData, CisUserData, DataNotFound, DataNotUpdated, DatabaseError}
  ...
  def submitDeductionPeriod(taxYear: Int, employerRef: String, user: User, deductionPeriod: Month): Future[Either[ServiceError, CisUserData]] ={
    cisSessionService.getSessionData(taxYear,employerRef,user).flatMap {
      case Right(Some(cisUserData)) =>
        ...
        val periodData: CYAPeriodData = cya.periodData.map(_.copy(deductionPeriod = deductionPeriod)).getOrElse(default)
      ...
      case _ => Future.successful(Left(DataNotFound))
    }
  }
}
