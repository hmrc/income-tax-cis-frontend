...
class TailoringDataConnector @Inject()(val http: HttpClientV2,
  ...
  def getExcludedJourneys(taxYear: Int, nino: String)
    ...
    http
      .get(url"$getExcludedJourneysUrl")
      .execute[ExcludedJourneysResponse]
  }
  ...
  def clearExcludedJourney(taxYear: Int, nino: String)
    ...
    http
      ...
      .withBody(Json.obj("journeys" -> Seq("cis")))
  ...
  def postExcludedJourney(taxYear: Int, nino: String)
    ...
    http
      .post(url"$postExcludedJourneyUrl")
      .withBody(Json.obj("journey" -> "cis"))
      .execute[PostExcludedJourneyResponse]
  }
